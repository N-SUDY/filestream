from Savory.vars import Var
from Savory.bot import StreamBot
from Savory.utils.human_readable import humanbytes
from Savory.utils.file_properties import get_file_ids
from Savory.server.exceptions import InvalidHash
import urllib.parse
import aiofiles
import logging
import aiohttp
...
async def render_page(id, secure_hash):
    ...
    if str(file_data.mime_type.split('/')[0].strip()) == 'video':
        async with aiofiles.open('Savory/template/req.html') as r:
        ...
        async with aiofiles.open('Savory/template/dl.html') as r:
   ...
   <p>
    ...
    <center>
        ...
        <button style="font-size: 20px; background-color: green; border-radius: 10px;" onclick="window.location.href = 'intent:{current_url}#Intent;package=video.player.videoplayer;S.title={file_data.file_name};end'">X player</button> &nbsp <br>
        <p>&nbsp</p>
